...
@Autonomous(name = "ShootDefenseRedMiddleAlt", group = "Encoder Autonomous")
public class ShootDefenseRedMiddleAlt extends OpMode {
    ...
    public void loop() {
       ...
       /* if (stage == 0) {//calibrates to 0
        ...
        if (stage == 8) {
            if (drive == null) {
                drive = new EncoderDrive(driveTrain, -600, 0.5);
                drive.run();
            }
            if (drive.isCompleted()) {
                drive.completed();
        ...
        if(stage == 16){
            if (drive == null){
                drive = new EncoderDrive(driveTrain, 1500, 0.75);
                drive.run();
            }
            if (drive.isCompleted()){
                drive.completed();
