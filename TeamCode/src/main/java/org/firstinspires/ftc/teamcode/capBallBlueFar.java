...
public class capBallBlueFar extends StateMachineOpMode {
    ...
    public void loop() {
        ...
        if (stage == 1) {
            ...
            {
                if (drive == null) {
                    drive = new EncoderDrive(driveTrain, 450, .75);
                    drive.run();
                }
        ...
        if (stage == 3) {
            if(moveType == 0){
                ...
                if (((gyroUtils.gyro.getHeading() > 10) && (gyroUtils.gyro.getHeading() < 320)) || turn.isCompleted()) {
                    driveTrain.stopRobot();
                    stage++;
                    time.reset();
                }
        ...
        if (stage == 5) {
            if (moveType == 0) {
                if (drive == null) {
                    drive = new EncoderDrive(driveTrain, 1000, .5);//1150, 0.5);
                    drive.run();
                    if (shoot > 0) {
        ...
        if (target.equals("Corner")) {
            if (moveType == 0){
                ...
                if (stage == 8) {
                    if(moveType == 0){
                        if (turn == null) {
                            turn = new EncoderTurn(driveTrain, 78, GyroUtils.Direction.CLOCKWISE);
                            turn.run();
                        }
                ...
                if (stage == 10) {
                    if(moveType == 0){
                        if (turn == null) {
                            turn = new EncoderTurn(driveTrain, 55, GyroUtils.Direction.CLOCKWISE);
                            turn.run();
                        }
